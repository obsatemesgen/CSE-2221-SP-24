...
public final class StringReassembly {
    ...
    public static String combination(String str1, String str2, int overlap) {
        ...
        int index = str2.indexOf(str1.substring(str1.length() - overlap));

        // Remove the overlapping part from str2 before concatenating
        String nonOverlappingStr2 = str2.substring(index + overlap);

        // Concatenate str1 with non-overlapping str2
        return str1 + nonOverlappingStr2;
    }
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet,
        ...
        // Check if str is a substring of any existing string
        boolean shouldAdd = true;
        Set<String> substringsToRemove = new Set2<>();
        for (String existingStr : strSet) {
            if (shouldAdd && existingStr.contains(str)) {
                shouldAdd = false;
            }
            if (!shouldAdd && str.contains(existingStr)) {
                substringsToRemove.add(existingStr);
            }
        }
    ...
    public static Set<String> linesFromInput(SimpleReader input) {
        ...
        // Instantiate Set2 to store the unique lines
        Set<String> strSet = new Set2<>();

        // Read each line from input
        while (!input.atEOS()) { // boolean modifier to report end of stream
    ...
    public static void printWithLineSeparators(String text, SimpleWriter out) {
        ...
        for (int i = 0; i < text.length(); i++) {
            ...
            if (ch == '~') {
                out.println(); // Print line separator
            } else {
                out.print(ch); // Print the character as it is
            }
        }

        // Ensure the last line is terminated with a newline
        out.println();
