...
public final class CryptoUtilities {
    ...
    public static void reduceToGCD(NaturalNumber n, NaturalNumber m) {
        ...
        while (!m.isZero()) {
            NaturalNumber temp = new NaturalNumber2(m); // Make a copy of m
            m.copyFrom(n.divide(temp)); // compute remainder of n/m and update m
            n.copyFrom(temp); // Update n with the previous value of m
        }
        // At this point, m = 0, and n contains the GCD
    }
    ...
    public static void powerMod(NaturalNumber n, NaturalNumber p,
        ...
        if (p.compareTo(new NaturalNumber2(0)) != 0) {
            n.setFromInt(1); // Anything to the power of 0 is 1
            return;
        } else if (p.compareTo(new NaturalNumber2(1)) == 0) {
            n.divide(m); // Optimization: Update n to n % m
            return;
        }
        // Initialize a natural number for holding the result of n^p % m
        NaturalNumber result = new NaturalNumber2(1);

        // Perform fast exponentiation
        while (p.compareTo(new NaturalNumber2(0)) > 0) {
            if (isEven(p)) {
                n.multiply(n); // Square n
                n.divide(m); // Reduce modulo m
                p.divide(new NaturalNumber2(2)); // Divide p by 2
            } else {
                result.multiply(n); // Multiply result by n
                result.divide(m); // Reduce modulo m
                p.decrement(); // Decrement p
            }
        }

        // Update n to the computed result
    ...
    public static boolean isWitnessToCompositeness(NaturalNumber w,
        ...
        // Store the original value of w
        NaturalNumber temp = new NaturalNumber2();
        temp.copyFrom(w);

        // Check if w^2 mod n = 1
        powerMod(w, new NaturalNumber2(2), n);
        boolean firstPart = (w.canConvertToInt() && w.toInt() == 1);

        // Restore the original value of w
        w.copyFrom(temp);
        // Check if w^(n-1) mod n != 1
        NaturalNumber nMinusOne = new NaturalNumber2(n);
        nMinusOne.decrement();
        powerMod(w, nMinusOne, n);
        boolean secondPart = !(w.canConvertToInt() && w.toInt() == 1);
    ...
    public static boolean isPrime2(NaturalNumber n) {
        ...
        if (n.equals(new NaturalNumber2(2))) {
            return true;
        }
        ...
        for (int i = 0; i < numCandidates; i++) {
            ...
            // Check if w^2 mod n = 1 or w^(n-1) mod n != 1
            NaturalNumber wPower2 = new NaturalNumber2(w);
            powerMod(wPower2, new NaturalNumber2(2), n);
            if (!wPower2.equals(new NaturalNumber2(1))) {
                return false;
            }

            NaturalNumber nMinus1 = new NaturalNumber2(n);
            nMinus1.decrement();
            NaturalNumber wPowerNMinus1 = new NaturalNumber2(w);
            powerMod(wPowerNMinus1, nMinus1, n);
            if (!wPowerNMinus1.equals(new NaturalNumber2(1))) {
                return false;
            }
        }
    ...
    public static void generateNextLikelyPrime(NaturalNumber n) {
        ...
        if (isEven(n)) {
            n.add(new NaturalNumber2(1)); // Make n odd
        }

        // Skip even numbers when searching for primes
        while (!isPrime2(n)) {
            n.add(new NaturalNumber2(2)); // Increment by 2 to consider only odd numbers
        }
    }
